...
class AdaptationTest {
    ...
    fun `at the cap the most redundant template in a multi-member bucket is pruned`() {
        // NORMAL bucket has the near-duplicate pair (0.0, 0.1); TIRED and ILL are sole-condition.
        val existing = listOf(
        ...
        val decision = decideAdaptation(existing, tmpl("new", VoiceCondition.NORMAL, 0.05f, 50), maxPerCommand = 4, distance = dist)
    ...
    @Test
    fun `maxPerCommand below 4 is rejected`() {
        val existing = listOf(tmpl("a", VoiceCondition.NORMAL, 0f, 1))
        try {
            decideAdaptation(existing, tmpl("new", VoiceCondition.NORMAL, 1f, 2), maxPerCommand = 3, distance = dist)
