...
| 2026-07-09 | **SOTA=1000 scorecard completed to 13/15 automated.** The five previously-`NOT_MEASURED` domains now have scripted mechanisms (`docs/testing/2026-07-09_sota-domains-completed.md`): D7 wake detection (`in_regime.py --emit`, ≤0.5 FA/hr point, PROXY/counts), D11 latency + D12 battery (`LatencyEval.kt`/`BatteryModel.kt`, host-scaled/first-principles, SIMULATED_DEVICE, **excluded from composite**), D13 enrollment efficiency (real TORGO sweep, 90.7% → band 950, counts). D10 language independence stays NOT_MEASURED (single-read Common Voice yields only chance-level rank-1) and is argued **by construction** (no LM in the shipped MFCC path; Zhang 2014; Picovoice 89.2% untuned) — see domain-bands §10. Composite unchanged: wall-dominated **`<600`** (FRR 75.7% @ FAR≤5% + ambient FA/hr). |
